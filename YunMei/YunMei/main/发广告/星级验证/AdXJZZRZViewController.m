...
@interface AdXJZZRZViewController ()
{
    UIButton *imageView;
}
...
@implementation AdXJZZRZViewController
@synthesize txtGroup,txtLincense,btnLincense,btnTax,btnPlace,btnCom;

- (void)viewDidLoad {
    [super viewDidLoad];
    txtGroup.layer.cornerRadius=8;
    txtGroup.layer.masksToBounds=YES;
    
    txtGroup.layer.borderWidth=1;
    txtGroup.layer.borderColor=[[UIColor grayColor] CGColor];
...
//    上传营业执照dusinessUrl，承诺书commitBookUrl，税务证书taxLicenseUrl，营业场所dusinessPlace
    ...
    if(txtLincense.text==nil)
    {
        [self showtips:@"请填写营业执照编号"];
        return;
    }
    ...
    if(btnPlace.imageView.image == nil)
    {
        [self showtips:@"请选择营业场所图片"];
        return;
    }
    if(btnCom.imageView.image == nil)
    {
        [self showtips:@"请选择承诺书图片"];
        return;
    }
    
    NSString *url=[NSString stringWithFormat:@"%@/client/act/comsumer/shopComfirm/add",LocalHostm];
    NSMutableDictionary *md=[[NSMutableDictionary alloc]init];
    [md setValue:txtLincense.text forKey:@"dusinessNum"];
    ...
    fileDic=[[NSMutableDictionary alloc]initWithCapacity:0];
    [fileDic setObject:@"place.png" forKey:@"fileName"];
    [fileDic setObject:@"image/png" forKey:@"type"];
    [fileDic setObject:@"placeUrl" forKey:@"name"];
    [fileDic setObject:UIImagePNGRepresentation(btnPlace.imageView.image) forKey:@"data"];
    [fileData addObject:fileDic];
    
    
    fileDic=[[NSMutableDictionary alloc]initWithCapacity:0];
    [fileDic setObject:@"com.png" forKey:@"fileName"];
    [fileDic setObject:@"image/png" forKey:@"type"];
    [fileDic setObject:@"commitBookUrl" forKey:@"name"];
    [fileDic setObject:UIImagePNGRepresentation(btnCom.imageView.image) forKey:@"data"];
    [fileData addObject:fileDic];
